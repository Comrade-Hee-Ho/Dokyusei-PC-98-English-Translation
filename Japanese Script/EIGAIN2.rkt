(mes
 ...
 (dict #\・ #\【 #\】 #\。 #\る #\ち #\な #\は #\い #\（ #\っ #\） #\て #\に
  #\ん #\ゃ #\あ #\、 #\か #\し #\う #\の #\？ #\た #\だ #\が #\！ #\よ #\く
  #\ー #\を #\ま #\こ #\映 #\画 #\と #\ろ #\も #\れ #\わ #\ら #\ふ #\じ #\君
  #\中 #\ぜ #\お #\で #\ね #\き #\ょ #\ど #\事 #\ぞ #\一 #\ぁ #\そ #\つ #\え
  #\へ #\寝 #\す #\時 #\間 #\女 #\ぺ #\反)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
        ...
        (<>
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((&& (== N 1) (== (: 50) 0))
                        (<>
                         (text #:color 7 "【" 0 "】（冗談じゃない・・・横に、ちはるちゃんがいるんだぜ。）")
                         (wait)))
                       ((&& (== N 1) (== (: 50) 1) (< (: 331) 10))
                        (<>
                         (text #:color 7 "【" 0 "】（うーん・・・。）")
               ...
               ((&& (== V 1) (== N 6) (== (: 12) 1))
                (<>
                 (text #:color 7 "【" 0 "】（くんか、くんか・・・髪の毛から、ほのかにシャンプーの香りがする。）")
                 (wait)))
               ((&& (== V 10) (== N 7) (== (: 13) 0))
                (<>
               ...
               ((&& (== V 10) (== N 7) (== (: 13) 1))
                (<>
                 (text #:color 7 "【" 0 "】何も反応がないと・・・やっても面白くないぞ。")
                 (wait)))
               ((&& (== V 1) (== N 8) (== (: 14) 0))
                (<>
               ...
               ((&& (== V 1) (== N 8) (== (: 14) 1))
                (<>
                 (text #:color 7 "【" 0 "】（彼女の顔に、スクリーンの光が反射してるな・・・。）")
                 (wait)))
               ((&& (== V 10) (== N 9) (== (: 15) 0))
                (<>
               ...
               ((&& (== V 10) (== N 9) (== (: 15) 1))
                (<>
                 (text #:color 7 "【" 0 "】（ちぇっ・・・反応がないと、何をしてもつまらんぞ。）")
                 (wait)))
               ((&& (== V 8) (== N 10) (== (: 16) 0))
                (<>
               ...
               ((&& (== V 1) (== N 3) (== (: 19) 1))
                (<>
                 (text #:color 7 "【" 0 "】（このヒゲを抜くと、方向感覚がなくなるのかな？）")
                 (wait)))
               ((&& (== V 1) (== N 4) (== (: 20) 0))
                (<>
               ...
               ((&& (== V 1) (== N 4) (== (: 20) 1))
                (<>
                 (text #:color 7 "【" 0 "】（うーん・・・涙を流して映画を見てるぞ。）")
                 (wait)))
               ((&& (== V 1) (== N 14) (== (: 21) 0))
                (<>
               ...
               ((&& (== V 1) (== N 14) (== (: 21) 1))
                (<>
                 (text #:color 7 "【" 0 "】映画館だぜ・・・あまり混んでないようだ。")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))