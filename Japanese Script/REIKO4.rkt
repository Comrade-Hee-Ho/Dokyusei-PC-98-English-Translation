(mes
 ...
 (dict #\・ #\【 #\】 #\。 #\ん #\い #\な #\、 #\は #\の #\て #\か #\子 #\う
  #\に #\っ #\あ #\だ #\麗 #\し #\で #\（ #\） #\る #\ま #\ら #\？ #\く #\人
  #\こ #\け #\を #\が #\た #\そ #\も #\ふ #\り #\ね #\さ #\ー #\と #\す #\よ
  #\れ #\和 #\ち #\ど #\え #\事 #\ン #\つ #\私 #\き #\女 #\気 #\せ #\服 #\わ
  #\何 #\家)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
 ...
 (cond ((== (: 554) 0)
        (<>
         ...
         (wait)
         (text #:color 2 "【女の人】毎朝、元気よくマンションから飛び出して行くのを見かけてましたし・・・それにほら、あなたが窓を開けた時に。")
         (wait)
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((&& (== N 1) (== (: 50) 0) (== (: 554) 0))
                       ...
                       ((&& (== N 3) (== (: 50) 0) (== (: 554) 0))
                        (<>
                         (text #:color 7 "【" 0 "】（そうだな・・・まだここで話をしていたいぞ。）")
                         (wait)))
                       ((&& (== N 3) (== (: 50) 1) (== (: 554) 0))
                        (<>
                       ...
                       ((== N 3)
                        (<>
                         (text #:color 7 "【" 0 "】（うーん・・・ずっとここで話をしていたい。）")
                         (wait)))
                       (else (<>)))))
               ((&& (== V 2) (== N 1) (== (: 10) 0) (== (: 554) 0))
                (<>
               ...
               ((&& (== V 2) (== N 1) (== (: 10) 1) (== (: 554) 0))
                (<>
                 ...
                 (wait)
                 (text #:color 2 "【麗子】この家に住んでるのは、今は私一人だけ・・・めったに出かけませんし、周りに住んでる方は気味が悪いかもしれませんね。")
                 (wait)
               ...
               ((&& (== V 1) (== N 3) (== (: 12) 1))
                (<>
                 (text #:color 7 "【" 0 "】（ソバージュかな？・・・麗子さんの雰囲気に合っているぜ。）")
                 (wait)))
               ((&& (== V 1) (== N 4) (== (: 13) 0))
                (<>
               ...
               ((&& (== V 1) (== N 4) (== (: 13) 1))
                (<>
                 (text #:color 7 "【" 0 "】（白く細い首だ・・・少しだけ汗が光ってる。）")
                 (wait)))
               ((&& (== V 1) (== N 5) (== (: 14) 0))
                (<>
               ...
               ((&& (== V 1) (== N 5) (== (: 14) 1))
                (<>
                 (text #:color 7 "【" 0 "】（和服の下って・・・ブラをつけるのかな？）")
                 (wait)))
               ((&& (== V 1) (== N 6) (== (: 15) 0))
                (<>
                 (text #:color 7 "【" 0 "】（うーん、純日本的な人妻だ・・・。）")
                 (wait)
                 (set-reg: 15 1)))
               ((&& (== V 1) (== N 6) (== (: 15) 1))
                (<>
                 (text #:color 7 "【" 0 "】（夏だというのに、和服を着て・・・暑くないのかな？）")
                 (wait)))
               ((&& (== V 1) (== N 7) (== (: 16) 0))
                (<>
