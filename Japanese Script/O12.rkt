(mes
 ...
 (dict #\・ #\【 #\】 #\い #\っ #\な #\。 #\て #\、 #\の #\ろ #\う #\ひ #\み
  #\ん #\が #\か #\た #\だ #\？ #\る #\し #\あ #\は #\に #\ゃ #\そ #\よ #\と
  #\く #\で #\ち #\り #\も #\ら #\ど #\き #\こ #\れ #\を #\！ #\（ #\事 #\）
  #\ー #\ま #\じ #\わ #\言 #\さ #\え #\す #\や #\彼 #\つ #\け #\氏 #\君 #\俺
  #\ね #\気 #\ぜ #\何 #\見 #\私 #\好 #\大 #\泣 #\話 #\聞 #\ば #\女 #\ょ #\お
  #\め #\手 #\同 #\情 #\持 #\男 #\方 #\上 #\今 #\色 #\子 #\び #\思 #\心 #\ッ
  #\チ #\げ #\生 #\せ #\タ #\人 #\々 #\時 #\ぞ #\直 #\ン #\ト #\ス #\明 #\プ
  #\対 #\ナ #\顔 #\分 #\所)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((&& (== N 1) (== (: 10) 0))
                       ...
                       ((&& (== N 1) (== (: 10) 1))
                        (<>
                         (text #:color 7 "【" 0 "】（あせるな、" 0 "・・・じっと待てば、必ず夜はやってくる。）")
                         (wait)))
                       (else (<>)))))
               ((&& (== V 2) (== N 1) (== (: 26) 0))
                (<>
               ...
               ((&& (== V 2) (== N 1) (== (: 26) 4))
                (<>
                 ...
                 (set-arr~ @ 7 0)
                 (proc 18)
                 (menu-show (<.> (text "かわいい顔") (text "大きい胸")
                             (text "上手そうな口")
                             (text "明るい性格")))
                 (cond ((== N 1)
                        (<>
                       ...
                       ((== N 4)
                        (<>
                         ...
                         (wait)
                         (text #:color 2 "【ひろみ】昔はもっと明るかったのよ・・・それが今の彼氏とつき合うようになって、どんどん暗くなっちゃったの。")
                         (wait)
                         (text #:color 7 "【" 0 "】気に入らないな・・・。")
                         (wait)
                         (text #:color 2 "【ひろみ】え？")
                         (wait)
                         (text #:color 7 "【" 0 "】俺はその彼氏を知らないけどさ・・・彼氏のせいにして色々と言うのは、聞いてる俺が気分よくないぜ。")
                         (wait)
               ...
               ((&& (== V 1) (== N 2) (== (: 10) 1))
                (<>
                 (text #:color 7 "【" 0 "】（俺って香水よりも、シャンプーの香りにぞくぞくしちゃうんだ・・・ひろみさんの髪は、何の香りがするんだろ？）")
                 (wait)))
               ((&& (== V 1) (== N 3) (== (: 11) 0))
                (<>
                 (text #:color 7 "【" 0 "】（こうやってまじまじと見ると、けっこう幼い顔つきだよな・・・くるみちゃんや美穂ちゃんとはまた違う、ロリロリタイプだ。）")
                 (wait)
                 (set-reg: 11 1)))
               ((&& (== V 1) (== N 3) (== (: 11) 1))
                (<>
                 (text #:color 7 "【" 0 "】（俺の事を見ているぞ・・・下心がバレないように気をつけるんだ。）")
                 (wait)))
               ((&& (== V 6) (== N 4) (== (: 12) 0))
                (<>
               ...
               ((&& (== V 1) (== N 6) (== (: 20) 3))
                (<>
                 (text #:color 7 "【" 0 "】（この部分をどう責めてやろうかな？・・・まずは、手で優しくナデナデして、その後で指を使って、それから舌でナメナメして・・・。）")
                 (wait)
               ...
               ((&& (== V 1) (== N 16))
                (<>
                 (text #:color 7 "【" 0 "】（いい天気だ・・・今日は外でおセックスをするのに、ぴったりの日だぜ。）")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))