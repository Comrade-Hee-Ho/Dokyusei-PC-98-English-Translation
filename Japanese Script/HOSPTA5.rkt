(mes
 ...
 (dict #\・ #\【 #\い #\】 #\よ #\や #\。 #\ん #\な #\っ #\か #\に #\う #\は
  #\だ #\、 #\ら #\し #\さ #\あ #\？ #\！ #\ー #\た #\て #\る #\が #\す #\ト
  #\そ #\デ #\で #\ろ #\く #\（ #\） #\れ #\ま #\と #\ど)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
 ...
 (while (== 1 1)
        (<>
         ...
         (cond ((== V 0)
               ...
               ((&& (== V 1) (== N 2))
                (<>
                 (text #:color 7 "【" 0 "】（奇麗だあ・・・黒い下着が思い出される。）")
                 (wait)))
               ((&& (== V 1) (== N 3))
                (<>
                 (text #:color 7 "【" 0 "】（奇麗な髪だな・・・それにつけても、黒い下着が思い出される。）")
                 (wait)))
               ((&& (== V 1) (== N 4))
                (<>
                 (text #:color 7 "【" 0 "】（なんて美しい肌なんだ・・・黒い下着がぴったりだぜ。）")
                 (wait)))
               ((&& (== V 1) (== N 5))
                (<>
                 (text #:color 7 "【" 0 "】（うーん、見るからに大きくて柔らかそうな胸だ・・・。）")
                 (wait)))
               ((&& (== V 1) (== N 6))
                (<>
                 (text #:color 7 "【" 0 "】（やよいさんだ・・・黒い下着がよく似合う女性だ。）")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))