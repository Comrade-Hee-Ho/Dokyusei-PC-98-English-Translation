(mes
 ...
 (dict #\・ #\い #\【 #\】 #\な #\。 #\子 #\真 #\は #\、 #\て #\か #\っ #\に
  #\う #\る #\ら #\ん #\の #\？ #\た #\し #\が #\と #\あ #\よ #\ゃ #\（ #\）
  #\で #\を #\君 #\ン #\だ #\じ #\も #\く #\れ #\そ #\こ #\わ #\ち #\さ #\ま
  #\事 #\行 #\け #\ど #\ポ #\コ #\チ #\病 #\相 #\談 #\先 #\生 #\言 #\ね #\見
  #\院 #\ー #\き #\え #\何 #\り #\や #\す #\！ #\入 #\め #\方)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
 (set-arr~ @ 7 0)
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((&& (== N 1) (== (: 600) 0))
                       ...
                       ((&& (== N 3) (== (: 600) 0))
                        (<>
                         (text #:color 7 "【" 0 "】（うーん・・・真子先生に相談しようかなあ、どうしようかなあ。）")
                         (wait)))
                       ((&& (== N 3) (== (: 600) 1))
                        (<>
                         (text #:color 7 "【" 0 "】（ここにいるより、病院に行った方がいいかな？）")
                         (wait)))
                       (else (<>)))))
               ((&& (== V 2) (== N 29) (== (: 10) 0) (== (: 600) 0))
                (<>
               ...
               ((&& (== V 2) (== N 29) (== (: 10) 2) (== (: 600) 0))
                (<>
                 (text #:color 3 "【真子】用がないのなら私は仕事をするけど・・・いいかしら？")
                 (wait)))
               ((&& (== V 2) (== N 29) (== (: 600) 1) (== (: 10) 0))
                (<>
               ...
               ((&& (== V 1) (== N 26) (== (: 600) 1))
                (<>
                 (text #:color 7 "【" 0 "】（ポコチンを見せたんだから、お返しに胸ぐらい見せて欲しいよな。）")
                 (wait)))
               ((&& (== V 1) (== N 27) (== (: 600) 0))
                (<>
                 (text #:color 7 "【" 0 "】（ポコチンの事を相談できる人は・・・真子先生しかいないよな。）")
                 (wait)))
               ((&& (== V 1) (== N 27) (== (: 600) 1))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生に相談してよかった・・・。）")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))