(mes
 ...
 (dict #\・ #\【 #\】 #\い #\。 #\な #\う #\子 #\真 #\っ #\だ #\は #\た #\て
  #\ん #\に #\、 #\か #\の #\ゃ #\ら #\よ #\と #\も #\る #\あ #\こ #\じ #\が
  #\し #\先 #\生 #\？ #\そ #\君 #\ち #\わ #\で #\ど #\気 #\く #\（ #\ー #\）
  #\ま #\病 #\け #\さ #\を #\痛 #\見)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((== N 1)
                       ...
                       ((== N 3)
                        (<>
                         (text #:color 7 "【" 0 "】（うーん・・・ここにいたいけど、真子先生の邪魔になるかな？）")
                         (wait)))
                       (else (<>)))))
               ((&& (== V 2) (== N 29) (== (: 10) 0) (== (: 603) 0))
                (<>
                 ...
                 (cond ((== N 1)
                        (<>
                         ...
                         (wait)
                         (text #:color 7 "【" 0 "】自分じゃわからないよ・・・もともとここだけは、自分の身体の中で一番コントロールがきかない部分だし。")
                         (wait)
               ...
               ((&& (== V 2) (== N 29) (== (: 10) 2))
                (<>
                 (text #:color 3 "【真子】さてと・・・私は仕事の続きをしないと。")
                 (wait)))
               ((&& (== V 1) (== N 30))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生の髪は・・・いつも奇麗だな。）")
                 (wait)))
               ((&& (== V 1) (== N 23))
                (<>
                 (text #:color 7 "【" 0 "】（うーん、あの目で見られると・・・身体が震えてきちゃうぜ。）")
                 (wait)))
               ((&& (== V 1) (== N 24))
                (<>
               ...
               ((&& (== V 1) (== N 25))
                (<>
                 (text #:color 7 "【" 0 "】（赤いネクタイだ・・・亜子さんとお揃いだな。）")
                 (wait)))
               ((&& (== V 1) (== N 26))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生のバストは・・・８５センチぐらいありそうだ。）")
                 (wait)))
               ((&& (== V 1) (== N 27))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生だ・・・大人の雰囲気があふれ出ている。）")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))