(mes
 ...
 (dict #\・ #\る #\【 #\】 #\ん #\く #\。 #\な #\み #\の #\い #\に #\て #\う
  #\、 #\っ #\ゃ #\？ #\か #\だ #\こ #\あ #\も #\と #\ち #\は #\（ #\） #\を
  #\り #\え #\さ #\た #\れ #\一 #\よ #\で #\が #\じ #\哉 #\言 #\ぜ #\ら #\そ
  #\け #\し)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
         ...
         (cond ((== V 0)
               ...
               ((&& (== V 2) (== N 1) (== (: 12) 0))
                (<>
                 ...
                 (wait)
                 (text #:color 6 "【くるみ】こんなに何度も電話して、何度も一哉君の家に行ってるのに・・・タイミングが悪いだけとは、とてもじゃないけど思えないもの。")
                 (wait)
                 (set-reg: 12 1)))
               ((&& (== V 2) (== N 1) (== (: 50) 0))
                (<>
               ...
               ((&& (== V 2) (== N 1))
                (<>
                 (text #:color 6 "【くるみ】ごめんね・・・" 0 "さん、変な事を頼んで。")
                 (wait)))
               ((&& (== V 1) (== N 2))
                (<>
               ...
               ((&& (== V 1) (== N 3))
                (<>
                 (text #:color 7 "【" 0 "】（表情は笑ってるように見えるが・・・寂しそうな笑いだな。）")
                 (wait)))
               ((&& (== V 1) (== N 4))
                (<>
                 (text #:color 7 "【" 0 "】（このリボンを結んでる時も・・・一哉の事を考えてるんだろうな。）")
                 (wait)))
               ((&& (== V 1) (== N 5))
                (<>
                 (text #:color 7 "【" 0 "】（一哉のやつ・・・この肌に触れずに、夏子さんに乗り換えるつもりか？）")
                 (wait)))
               ((&& (== V 1) (== N 6))
                (<>
                 (text #:color 7 "【" 0 "】（見方を変えると、胸の中を見せてるようにも見えるぞ・・・。）")
                 (wait)))
               ((&& (== V 1) (== N 7))
                (<>
