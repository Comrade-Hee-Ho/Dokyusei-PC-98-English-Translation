(mes
 ...
 (dict #\・ #\い #\な #\【 #\】 #\。 #\っ #\る #\て #\に #\、 #\子 #\し #\の
  #\真 #\う #\こ #\ら #\も #\（ #\と #\が #\あ #\だ #\） #\は #\先 #\ん #\か
  #\生 #\？ #\た #\を #\気 #\れ #\よ #\ま #\わ #\や #\そ #\す #\で)
 (set-arr~ @ 7 1)
 (set-arr~ @ 3 0)
 (set-arr~ @ 2 45056)
 (set-arr~ @ 7 0)
         ...
         (cond ((== V 0)
                (<>
                 ...
                 (cond ((== N 1)
                       ...
                       ((== N 2)
                        (<>
                         (text #:color 7 "【" 0 "】中に入るって・・・もう入ってるだろ？")
                         (wait)))
                       ((== N 3)
                        (<>
               ...
               ((&& (== V 2) (== N 29) (== (: 10) 1))
                (<>
                 ...
                 (wait)
                 (text #:color 3 "【真子】" 0 "君、その絵はちゃんと消しておきなさい・・・夏休みが終ったら放送部員がこの部屋に来るんですから。")
                 (wait)
                 (set-reg: 10 2)))
               ((&& (== V 2) (== N 29) (== (: 10) 2))
                (<>
               ...
               ((&& (== V 1) (== N 23))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生の顔は・・・美しい。）")
                 (wait)))
               ((&& (== V 1) (== N 30))
                (<>
                 (text #:color 7 "【" 0 "】（髪の毛がフワフワしてて・・・綿帽子みたいな髪だ。）")
                 (wait)))
               ((&& (== V 1) (== N 24))
                (<>
               ...
               ((&& (== V 1) (== N 27))
                (<>
                 (text #:color 7 "【" 0 "】（真子先生って、男を迷わす魅力がある・・・もし先生に知性と優しさがなかったら、単なる悪女になってしまいそうだ。）")
                 (wait)))
               ((&& (== V 1) (== N 28))
                (<>
                 (text #:color 7 "【" 0 "】（いつも同じポーズのような気がするけど・・・気のせいだろうな。）")
                 (wait)))
               (else (<>)))))
 (set-arr~ @ 7 1))