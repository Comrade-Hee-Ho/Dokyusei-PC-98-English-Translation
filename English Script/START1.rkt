(mes
 ...
 (set-arr~ @ 7 0)
 (proc 18)
 (menu-show (<.> (text "Desktop") (text "98Note")
             (text "Hard Disk")))
 (if-else (== N 2)
          (<> (set-arr~ @ 9 (// (~ @ 9) 64)))
          (<> (set-arr~ @ 9 (&& (~ @ 9) 65471))))
 (set-arr~ @ 7 0)
 (proc 18)
 (menu-show (<.> (text "Start") (text "Continue")))
 (if-else (== N 1)
          (<>)
          (<>
           (set-arr~ @ 7 0)
...
a very independent 18-year old.")
 (wait)
 (text #:color 7 "Everyone is filled with respect, and calls me
" 0 "... Wait, wait, isn't that just my name?")
 (wait)
